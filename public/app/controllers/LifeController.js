    ...
    function($scope, $http, $route, $routeParams, $interval) {
        ...
        if ($scope.$routeParams.userId) {
            ...
            $http.get('api/users/' + userId)
               .success(function(data) {
                   ...
                   $scope.user = data;
                   $scope.user.bod_string = moment($scope.user.bod)
                                                .format("dddd, MMMM Do YYYY");

                    var year = moment($scope.user.bod).get('year');

                    $http.get('/api/population/' + year)
                        .success(function (data) {
                            $scope.worldPopulation = Number(data).toLocaleString();
                        });

                    $http.get('/api/life_expectancy/' + $scope.user.country + "/" +
                                                        year + "/" +
                                                        $scope.user.sex)
                        .success(function (data) {
                            ...
                            $scope.user.livesDayPercentage =
                                math.round(($scope.user.age*100)/life_expectancy, 2);

                            setupCanvas($scope.user.livesDayPercentage);
        ...
        function drawCanvas(percentageLived) {
            centerX = canvasWidth / 2;
    		    ...
    		    context.arc(centerX, centerY, radius - 10,
